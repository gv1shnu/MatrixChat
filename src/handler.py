...
class Handler:
    ...
    def get_all_users(self):
        try:
            select_all_query = "SELECT email FROM users"
            self.cursor.execute(select_all_query)
            users = self.cursor.fetchall()
            return users
        except mysql.connector.Error as err:
            logger.info(f"Error during fetching users: {err}")
    ...
    def get_all_messages_for(self, username):
        try:
            select_all_query = (
                "SELECT message_id, sender, message "
                "FROM messages "
                "WHERE receiver = %s "
                "AND unread=1"
            )
    ...
    def insert_user(self, user_data):
        insert_query = (
            "INSERT INTO users (u_id, full_name, email, profile_pic) "
            "VALUES (%s, %s, %s, %s)"
        )
